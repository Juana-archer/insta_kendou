...
class InstagramAuth:
    ...
    def _generate_fresh_device_info(self):
        ...
        # GÉNÉRER LE NOUVEAU USER-AGENT WEB ALÉATOIRE
        device_info['user_agent'] = self._generate_random_user_agent()
    ...
    def _generate_random_user_agent(self) -> str:
        """
        Générer un User-Agent web aléatoire comme demandé.

        Returns:
            Une chaîne User-Agent aléatoire
        """
        import random
        import string
    ...
    def _setup_base_headers(self, device_info):
        ...
        # Extraire la version Android depuis le User-Agent généré
        user_agent = device_info.get("user_agent", "")
        android_version = "14"  # Valeur par défaut
        
        if "Android" in user_agent:
            try:
                # Extraire la version Android du User-Agent
                import re
        ...
        # Extraire le modèle (device_code + device_number)
        model = "K"  # Valeur par défaut
        if "Android" in user_agent:
            try:
                match = re.search(r'Android \d+; ([A-Z0-9]+)\)', user_agent)
                if match:
                    model = match.group(1)
            except:
                pass
        ...
        self.base_headers = {
            ...
            "sec-ch-ua-full-version-list": '"Chromium";v="144.0.0.0", "Not=A?Brand";v="24.0.0.0", "Google Chrome";v="144.0.0.0"',
            "sec-ch-prefers-color-scheme": "light",
            "user-agent": user_agent,  # Utilise le User-Agent généré aléatoirement
            "accept-language": "fr-FR,fr;q=0.9,en-US;q=0.8,en;q=0.7"
        }
