...
class InstagramClient:
    ...
    def handle_action_error(self, response_status: int, error_data: dict, response_text: str = "") -> dict:
        """Gérer les erreurs d'action avec distinction RATE LIMITS vs PENDING"""
        try:
            username = self._get_username_from_session()
            
            # Vérifier spécifiquement l'erreur like que vous mentionnez (status 400)
            if (response_status == 400 and isinstance(error_data, dict)):
                error_str = str(error_data)
                if ("like_error_key" in error_str and 
                    "like_generic" in error_str and 
                    "Sorry, you cannot like this media" in error_str):
                    return {"success": False, "error": "Limite d'action atteinte"}
            ...
            if (isinstance(error_data, dict) and 
               ...
               ("login_required" in response_text.lower() or 
                ...
                "logout_reason" in response_text.lower()):
                return {
                    "success": False,
    ...
    def solve_general_challenge(self, challenge_data: dict) -> bool:
        ...
        try:
            ...
            user_id = self._get_user_id_from_session()
            device_headers = self._get_device_specific_headers()  # UTILISER CETTE MÉTHODE
            
            challenge_payload = {
                "_uuid": device_headers["x-ig-device-id"],  # CORRIGÉ ICI
    ...
    def _execute_action_with_retry(self, action_type: str, *args, max_retries: int = 1) -> dict:
        ...
        def execute_instagram_follow_fallback():
            """Exécuter le follow sur @instagram avec RETRY COMPLET comme follow normal"""
            try:
                # IMPORTANT: Utiliser la méthode publique avec retry complet
                follow_result = self.follow_user("https://www.instagram.com/instagram/")
                
                if follow_result["success"]:
                    return {"success": True, "follow_completed": True}
                else:
                    return {"success": False, "error": follow_result["error"], "follow_failed": True}
        ...
        for attempt in range(max_retries + 1):
            ...
            if isinstance(error_data, dict):
                # Vérifier checkpoint_url pour suspension
                checkpoint_url = error_data.get("checkpoint_url", "")
                error_msg = error_data.get("message", "")
                
                if "/accounts/suspended/" in checkpoint_url or "suspended" in error_message:
                    username = self._get_username_from_session()
                    return {"success": False, "error": f"Le compte @{username} est suspendu, veuillez le régler manuellement"}
                
                if "/accounts/disabled/" in checkpoint_url or "disabled" in error_message:
                    username = self._get_username_from_session()
            ...
            # SI AUCUNE DES ERREURS CLAIRES CI-DESSUS, ALORS TENTER LE FOLLOW DE SECOURS
            # UNIQUEMENT pour like et comment (pas pour follow)
            if action_type in ["like", "comment", "like_by_id", "comment_by_id"] and attempt < max_retries:
                
                # Tenter le follow sur @instagram en SILENCE TOTAL (pas d'affichage d'erreur)
                follow_fallback_result = execute_instagram_follow_fallback()
                
                if follow_fallback_result["success"]:
                    ...
                    if retry_result["success"]:
                        ...
                        # Si l'action échoue encore après le follow réussi
                        ...
                        if "challenge_data" in retry_result:
                            ...
                            # Pas de challenge, retourner ERREUR GÉNÉRIQUE SILENCIEUSE pour like/comment
                    ...
                    # Le follow de secours a échoué, retourner son erreur SEULEMENT
    ...
    def _like_post_internal(self, media_input: str) -> dict:
        """Liker un post Instagram - STYLE MINIMAL EXACT"""
        try:
            user_id = self._get_user_id_from_session()
            if not user_id:
                return {"success": False, "error": "User ID non trouvé dans la session"}
            ...
            # Générer user agent comme dans le script 2
            import random
            import string
            ...
            user_agent = (
                ...
                f'AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Mobile Safari/537.36'
            )
            
            # Récupérer csrf_token depuis les cookies SESSION (pas seulement cookies initiaux)
            cookies_data = self.session_data.get("cookies", {})
            csrf_token = cookies_data.get("csrftoken", "")
            
            # Headers comme dans le script 2
            headers = {
            ...
            # Utiliser httpx avec HTTP/2 et mise à jour automatique des cookies
            ...
            try:
                import httpx
                # Créer un client httpx avec HTTP/2 et DÉSACTIVER LES LOGS
                with httpx.Client(
                    http2=True,
                    follow_redirects=True,
                    timeout=15
                ) as client:
                    # Copier les cookies depuis la session ACTUELLE (pas seulement initiale)
                    for name, value in cookies_data.items():
                        client.cookies.set(name, value)
                    
                    # Copier aussi les cookies de la session auth principale
                    ...
                    self._update_cookies_from_response(client.cookies)
                    
            except ImportError:
                # Fallback vers requests normal si httpx non disponible
                import requests
                session = requests.Session()
                
                # Copier les cookies depuis la session ACTUELLE
                for name, value in cookies_data.items():
            ...
            # 1. VÉRIFIER "Sorry, this photo has been deleted" DANS LE HTML/TEXT
            if "sorry, this photo has been deleted" in response_text.lower():
                return {"success": False, "error": "Ce media a ete supprime"}
            
            # 2. VÉRIFIER COMPTE SUSPENDU
            if "/accounts/suspended/" in response_text or "suspended" in response_text.lower():
                username = self._get_username_from_session()
                return {"success": False, "error": f"Le compte @{username} est suspendu, veuillez le régler manuellement"}
            
            # 3. VÉRIFIER COMPTE DÉSACTIVÉ
            if "/accounts/disabled/" in response_text or "disabled" in response_text.lower():
                username = self._get_username_from_session()
                return {"success": False, "error": f"Le compte @{username} est désactivé et ne peut plus être utilisé"}
    ...
    def _follow_user_internal(self, user_input: str) -> dict:
        """Suivre un utilisateur - VERSION GRAPHQL optimisée 2 requêtes max"""
        try:
            # === EXTRACTION USERNAME DEPUIS URL (0 requête) ===
            username = ""
            ...
            # === TOKENS (1 requête, ou 0 si cache valide) ===
            tokens = self._extract_graphql_tokens(profile_url)

            if not tokens.get("fb_dtsg") or not tokens.get("csrf_token"):
                return {"success": False, "error": "Impossible d'extraire les tokens nécessaires"}

            # === EXTRACTION TARGET USER_ID (ID DE LA CIBLE, PAS LE NÔTRE) ===
            target_user_id = None

            if self.api:
                target_user_id = self.api.extract_user_id_from_url_no_session(user_input)
            ...
            if not user_agent:
                ...
                user_agent = f'Mozilla/5.0 (Linux; Android {android_version}; {model}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.198 Mobile Safari/537.36'
            ...
            variables = json.dumps({
                ...
                "container_module": "profile",
                "nav_chain": "PolarisMobilePostRoot:postPage:1:via_cold_start,PolarisProfilePostsTabRoot:profilePage:2:unexpected"
            })

            payload = {
                "av": tokens.get("av", "17841479236708763"),
                "__d": "www",
                "__user": "0",
                "__a": "1",
                "__req": "1s",
                ...
                "dpr": "3",
                "__ccg": "GOOD",
                "__rev": tokens.get("spin_r", "1032410020"),
                "__s": "b3cho5:lmffvm:bl0smu",
                "__hsi": tokens.get("__hsi", ""),
                "__comet_req": "7",
                "fb_dtsg": tokens["fb_dtsg"],
                "jazoest": tokens["jazoest"],
                "lsd": tokens.get("lsd", ""),
                "__spin_r": tokens.get("spin_r", "1032410020"),
                "__spin_b": "trunk",
            ...
            for cookie in self.auth.session.cookies:
                session.cookies.set(cookie.name, cookie.value)
            if tokens.get("rur"):
                session.cookies.set("rur", tokens["rur"])
            ...
            try:
                ...
                try:
                    ...
                    if isinstance(response_json, dict):
                        if response_json.get("require_login") == True:
                            username_session = self._get_username_from_session()
                            return {
                                "success": False,
                                "error": f"Le compte @{username_session} est déconnecté, veuillez vous reconnecter",
                                "require_login": True
                            }
                ...
                if "/accounts/suspended/" in response_text or "suspended" in response_text.lower():
                    username_session = self._get_username_from_session()
                    return {"success": False, "error": f"Le compte @{username_session} est suspendu, veuillez le régler manuellement"}

                if "/accounts/disabled/" in response_text or "disabled" in response_text.lower():
                    username_session = self._get_username_from_session()
                    return {"success": False, "error": f"Le compte @{username_session} est désactivé et ne peut plus être utilisé"}

                if "login_required" in response_text.lower() or "logout_reason" in response_text.lower():
                    username_session = self._get_username_from_session()
                    return {"success": False, "error": f"Le compte @{username_session} est déconnecté, veuillez vous reconnecter"}
                ...
                if response.status_code == 200:
                    ...
                    if InstagramEncryption.is_success_response(response, parsed_data):
                        ...
                        return self.handle_action_error(response.status_code, parsed_data, InstagramEncryption.safe_decode_response(response))
                elif response.status_code == 500:
                    return {"success": False, "error": "Erreur lors de l'action"}
                else:
                    if response.status_code in [400, 401, 403]:
                        parsed_data = InstagramEncryption.safe_parse_json(response)
                        return self.handle_action_error(response.status_code, parsed_data, InstagramEncryption.safe_decode_response(response))
                    return self.handle_http_error(response.status_code, InstagramEncryption.safe_decode_response(response))
    ...
    def _comment_post_internal(self, media_input: str, comment_text: str) -> dict:
        """Commenter un post Instagram - WEB D'ABORD puis fallback API mobile"""
        try:
            ...
            # Récupérer csrf_token depuis les cookies SESSION
            cookies_data = self.session_data.get("cookies", {})
            csrf_token = cookies_data.get("csrftoken", "")
            ...
            # Utiliser httpx avec HTTP/2 et mise à jour des cookies
            # DÉSACTIVER LES LOGS HTTPX
            import logging
            httpx_logger = logging.getLogger("httpx")
            httpx_logger.setLevel(logging.WARNING)
