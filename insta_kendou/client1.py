...
class InstagramClient:
    ...
    def _execute_action_with_retry(self, action_type: str, *args, max_retries: int = 1) -> dict:
        ...
        for attempt in range(max_retries + 1):
            ...
            # Si échec pour les actions like et comment, tenter le follow de secours SILENCIEUSEMENT
            ...
            if "challenge_data" not in result:
                # Pour like/comment, retourner erreur générique silencieuse
    ...
    def _follow_user_internal(self, user_input: str) -> dict:
        """Suivre un utilisateur - STYLE MINIMAL EXACT"""
        try:
            user_id = None
            ...
            # Données follow MINIMALES comme exemple
            follow_data = {
            ...
            # Headers MINIMAUX exactement comme exemple
            headers = {
                "user-agent": device_headers["user-agent"],
                "accept": "*/*",
                "connection": "keep-alive",
                ...
                "authorization": self._get_auth_token(),
                "ig-u-ds-user-id": current_user_id,
                "ig-u-ig-direct-region-hint": f"LLA,{current_user_id},{int(time.time() + 30*24*3600)}.3882394:01f7bae7d8b131877d8e0ae1493252280d72f6d0d554447cb1dc9049b6b2c507c08605b7",
                "ig-u-shbid": f"12695,{current_user_id},{int(time.time() + 30*24*3600)}.3882394:01f778d9c9f7546cf3722578fbf9b85143cd6e5132723e5c93f40f55ca0459c8ef8a0d9f",
                "ig-u-shbts": f"{int(time.time())},{current_user_id},{int(time.time() + 30*24*3600)}.3882394:01f7ace11925d0388080078d0282b75b8059844855da27e23c90a362270fddfb3fae7e28",
                "ig-u-rur": f"RVA,{current_user_id},{int(time.time() + 30*24*3600)}.3882394:01f7f627f9ae4ce2874b2e04463efdb184340968b1b006fa88cb4cc69a942a04201e544c",
                "content-type": "application/x-www-form-urlencoded; charset=UTF-8"
            }
    ...
    def _like_post_internal(self, media_input: str) -> dict:
        ...
        try:
            ...
            # Données like MINIMALES comme exemple
            like_data = {
                "_uuid": device_headers["x-ig-device-id"],
                "device_id": device_headers["x-ig-android-id"], 
                "radio_type": self._get_radio_type(),
            ...
            headers = {
                ...
                "ig-intended-user-id": user_id,
                "x-ig-nav-chain": "9MV:self_profile:2,ProfileMediaTabFragment:self_profile:3,9Xf:self_following:4",
                "x-ig-salt-ids": str(self._get_salt_ids()[0]) if self._get_salt_ids() else "1061244807",
                "authorization": self._get_auth_token(),
                "ig-u-ds-user-id": user_id,
                "ig-u-ig-direct-region-hint": self._get_ig_headers().get("ig-u-rur", "").replace("CLN", "LLA").replace("RVA", "LLA"),
                "ig-u-shbid": f"12695,{user_id},{int(time.time() + 30*24*3600)}.1560874:01f778d9c9f7546cf3722578fbf9b85143cd6e5132723e5c93f40f55ca0459c8ef8a0d9f",
                "ig-u-shbts": f"{int(time.time())},{user_id},{int(time.time() + 30*24*3600)}.1560874:01f7ace11925d0388080078d0282b75b8059844855da27e23c90a362270fddfb3fae7e28",
                "ig-u-rur": f"RVA,{user_id},{int(time.time() + 30*24*3600)}.1560874:01f7f627f9ae4ce2874b2e04463efdb184340968b1b006fa88cb4cc69a942a04201e544c",
                "content-type": "application/x-www-form-urlencoded; charset=UTF-8"
            }
            ...
            if response.status_code == 200:
                ...
                # *** CHANGEMENT ICI : RETOUR SILENCIEUX AU LIEU DE handle_http_error ***
                return {"success": False, "error": "Ce media a ete supprime"}
    ...
    def _comment_post_internal(self, media_input: str, comment_text: str) -> dict:
        ...
        try:
            ...
            # ÉTAPE 2: TENTER MÉTHODE WEB D'ABORD (GraphQL)
            try:
                web_result = self._comment_post_web_graphql(media_id, comment_text, media_input)
                if web_result["success"]:
                    return web_result
                # Si échec web, passer silencieusement au fallback mobile
            except:
                # Toute erreur web = passer silencieusement au fallback mobile
                pass
            
            # ÉTAPE 3: FALLBACK - Méthode web API simple actuelle
            user_id = self._get_user_id_from_session()
            if not user_id:
                return {"success": False, "error": "User ID non trouvé dans la session"}
            
            try:
                # Récupérer device settings réels depuis la session
                device_settings = self.session_data.get("device_settings", {})
                
                # Construire user agent réel depuis la session
                real_user_agent = device_settings.get("user_agent", "Mozilla/5.0 (Linux; Android 12; SM-G991B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36")
                
                # Récupérer la page pour extraire csrf_token
                web_response = self.auth.session.get(
                    media_input,
                    headers={"user-agent": real_user_agent},
                    timeout=10
                ...
                if web_response.status_code == 200:
                    ...
                    if csrf_match:
                        ...
                        # Mise à jour des cookies avec csrf_token
                        cookies = self.session_data.get("cookies", {})
                        cookies["csrftoken"] = csrf_token
                        
                        # Restaurer tous les cookies dans la session
                        for name, value in cookies.items():
                            self.auth.session.cookies.set(name, value)
                        
                        # Requête de commentaire web
                        response = self.auth.session.post(
                            f"https://www.instagram.com/api/v1/web/comments/{media_id}/add/",
                            headers=web_headers,
                            data=web_comment_data,
